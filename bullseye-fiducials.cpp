...
int main( int argc, char* argv[] )
{
	...
	if ( config.input_file == "" && config.camera == false ) {
		...
		if ( config.output_file != "" ) {
			draw_borders( config.input_file, config.output_file, output_json, config.debug );	
			draw_corner_arrows( config.output_file, config.output_file, output_json, config.debug );	
		}
	}
